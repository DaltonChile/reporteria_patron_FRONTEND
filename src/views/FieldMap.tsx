...
export default function FieldMap() {
  ...
  const cuarteles = [
    { id: 1, name: 'Cuartel Norte', status: 'Cosecha', x: 20, y: 10, w: 30, h: 40, color: 'fill-green-400' },
    { id: 2, name: 'Cuartel Sur', status: 'Poda', x: 60, y: 10, w: 30, h: 40, color: 'fill-blue-400' },
    { id: 3, name: 'Cuartel Este', status: 'Riego', x: 20, y: 60, w: 70, h: 30, color: 'fill-yellow-400' },
  ];
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-80 bg-white rounded-2xl shadow-sm border border-slate-200 p-6 self-start">
          <h3 className="text-lg font-bold text-brand-blue border-b border-slate-100 pb-3 mb-4">
            {activeCuartel ? cuarteles.find(c => c.id === activeCuartel)?.name : 'Selecciona un cuartel'}
          </h3>
          ...
          {activeCuartel ? (
            <div className="space-y-4">
              <div>
                ...
                <span className="text-sm font-medium text-slate-800">{cuarteles.find(c => c.id === activeCuartel)?.status}</span>
              </div>
              <div>
